...
```bash
# From aim-browser repo root (starts minimized, stops/closes Chromium when done)
...
# Reuse already-running daemon and leave it up for more queries
node aim-google-ai.skill/scripts/run.js --no-start --keep-open "best static site generators"

# Watch mode (show browser window; still closes after session unless --keep-open)
...
- Default session policy: **stop/close Chromium after every run** so no window is left on the desk. Use `--keep-open` only when reusing a long-lived daemon.  
