 ...
 *   node aim-google-ai.skill/scripts/run.js --query "..." [--visible] [--no-start] [--keep-open]
 *   node aim-google-ai.skill/scripts/run.js --screenshot /tmp/out.png "query"
 *
 * Default: stop Chromium daemon when the session ends (window closed).
 * Pass --keep-open to leave the daemon running for reuse.
 */
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { AimBrowser, startDaemon, stopDaemon, checkDaemon } from '../../src/index.js';
import { cacheGet, cacheSet } from '../../src/skill-utils.js';
...
function parseArgs(argv) {
  const opts = {
    ...
    visible: false,
    noStart: false,
    // Operator preference: close Chromium after every session (no leftover window).
    stopAfter: true,
    timeoutMs: 90000,
    screenshot: null,
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--stop-after') opts.stopAfter = true; // default; kept for compatibility
...
async function main() {
  ...
  --stop-after         Stop daemon when finished (default; explicit for clarity)
  ...
  const cacheParts = { query: opts.query, mode: 'udm50' };
  if (!opts.noCache) {
    const hit = cacheGet('aim-google-ai', cacheParts);
    if (hit) {
      console.error('[aim-google-ai] Cache hit');
      printMarkdown(hit, opts.query);
      return;
    }
  }
  ...
  try {
    ...
    cacheSet('aim-google-ai', cacheParts, data);
    ...
    if (opts.stopAfter) {
      console.error('[aim-google-ai] Stopping daemon (close after session)…');
      try { stopDaemon(); } catch { /* */ }
    } else {
      console.error('[aim-google-ai] Leaving daemon running (--keep-open)');
    }
  }
}
