...
|----------|----------------------------|
| Pure `--headless=new` as default | Undermines the “real headed browser” reliability thesis |
...
Use **minimized by default** day-to-day; implement **Phase B Xvfb “seamless headed”** next so agents stay headed for realistic browser behavior **without ever mapping a window on `:0`**.
